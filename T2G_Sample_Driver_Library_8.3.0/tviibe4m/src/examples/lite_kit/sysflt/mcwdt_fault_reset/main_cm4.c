...
void irqFaultReport0Handler(void)
{
    ...
    if(status != CY_SYSFLT_NO_FAULT)
    {
        if(status == CY_SYSFLT_SRSS_FAULT_MCWDT0) // MCWDT0 fault
        {
            ...
            {
                // won't reach here
                Cy_GPIO_Inv(CY_CB_LED_PORT, CY_CB_LED_PIN); // subcounter 0 upper limit fault
            }
            ...
            {
                // Immediately after LED1 (as LED2 on hdr file) light up, the MCU will be reset.
                Cy_GPIO_Inv(CY_CB_LED2_PORT, CY_CB_LED2_PIN); // subcounter 1 upper limit fault
            }
        }
    }
...
int main(void)
{
    ...
    user_led_port_pin_cfg.hsiom  = CY_CB_LED_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_CB_LED_PORT, CY_CB_LED_PIN, &user_led_port_pin_cfg);

    user_led_port_pin_cfg.hsiom  = CY_CB_LED2_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_CB_LED2_PORT, CY_CB_LED2_PIN, &user_led_port_pin_cfg);

    user_led_port_pin_cfg.hsiom  = CY_CB_LED3_PIN_MUX;
    Cy_GPIO_Pin_Init(CY_CB_LED3_PORT, CY_CB_LED3_PIN, &user_led_port_pin_cfg);

    // Not Lighting LED2 (as LED3 on hder file) indicates reset occurred.
    Cy_SysTick_DelayInUs(1000000);

    Cy_GPIO_Inv(CY_CB_LED3_PORT, CY_CB_LED3_PIN);
