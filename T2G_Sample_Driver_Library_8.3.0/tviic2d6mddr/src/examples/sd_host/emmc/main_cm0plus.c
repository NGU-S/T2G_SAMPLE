...
/* User configurable parameter, please check the possibility and select */
#define CY_SD_HOST_SRC_PLL_PATH_NO      (7u)
#define CY_SD_HOST_SRC_CLOCK            100000000       /* 100MHz */
#define CY_SD_HOST_BUS_SPEED            CY_SD_HOST_BUS_SPEED_DDR50
#define CY_SD_HOST_BUS_CLOCK            CY_SD_HOST_CLK_50M
...
static void ChangePLLFrequency(uint32_t outputFreq)
{
    ...
    CY_ASSERT(Cy_SysClk_PllDisable(CY_SD_HOST_SRC_PLL_PATH_NO) == CY_SYSCLK_SUCCESS);
    CY_ASSERT(Cy_SysClk_ClkPathSetSource(CY_SD_HOST_SRC_PLL_PATH_NO, CY_SYSCLK_CLKPATH_IN_ECO) == CY_SYSCLK_SUCCESS);
    CY_ASSERT(Cy_SysClk_PllConfigure(CY_SD_HOST_SRC_PLL_PATH_NO , &pll200_1_Config) == CY_SYSCLK_SUCCESS);
    CY_ASSERT(Cy_SysClk_PllEnable(CY_SD_HOST_SRC_PLL_PATH_NO, 10000ul) == CY_SYSCLK_SUCCESS);
    return;
}
...
int main(void)
{
    ...
    /* Enable SDHC clock */
    Cy_SysClk_HfClkEnable(CY_SYSCLK_HFCLK_13);
    /* Update PLL clock for SDHC */
    ChangePLLFrequency(CY_SD_HOST_SRC_CLOCK);
    ...
    sd_host_context.busSpeed = CY_SD_HOST_BUS_SPEED;
    sd_host_context.busClock = CY_SD_HOST_BUS_CLOCK;
