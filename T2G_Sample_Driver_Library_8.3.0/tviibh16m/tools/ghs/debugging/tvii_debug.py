...
#
# History:
# 2023-03-06 0.xx CHEY  >>> no history logging for early releases <<<
# ***********************************************************************************************/#
...
WORK_FLASH0_BASE_ADDRESS                   = 0x14000000
WORK_FLASH1_BASE_ADDRESS                   = 0x1c000000
...
def IsApplicationLinkedForFlash():
    ...
    if ( ((main_address >= CODE_FLASH0_BASE_ADDRESS)  and (main_address < (CODE_FLASH0_BASE_ADDRESS  + CODE_FLASH_SIZE_BYTES))) or
...
def CheckFlashSuccess(cmd_output):
    ...
    if cmd_output.find("Could not detect a flash device at") != -1:
        ...
        print("")
        print("In case of 'tviibh16m' you may need to update the probe driver / install a patch.")
        print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
# Execute a GHS 'flash burn' command with the given parameters
def ExecFlashBurnCmd(elf_file, base_addr_str):
    # GHS flash kernel must be run on CM0+ at the moment
    ExecDebugCmdViaCm0("flash burn -baseaddress=" + base_addr_str +
...
def EraseCodeFlash(offset=0, size=CODE_FLASH_SIZE_BYTES, macro=0):
    ...
    with tempfile.NamedTemporaryFile(delete = False) as temp_file_for_erase:
        try:
            ...
            flash_macro_base_address = CODE_FLASH0_BASE_ADDRESS if macro == 0 else CODE_FLASH1_BASE_ADDRESS
            # GHS flash kernel must be run on CM0+ at the moment
...
# Handle all required steps to program the contents of one application
def ProgramApplication(map_cmd_output):
    if PERMIT_SFLASH_USER_ROW_PROGRAMMING:
        sflash_permission_string = "&write_sflash=1"
    else:
        sflash_permission_string = ""

    elf_file_search_result = re.search('^Module \((.+)\):$', map_cmd_output, re.MULTILINE)
    if elf_file_search_result:
        elf_file = "\"" + elf_file_search_result.group(1) + "\""
    else:
        ExitScriptWithError("ERROR: Could not parse 'map' command output for ELF file path!")
    
    # Check the 2 most significant nibbles of the section start address to find out for whether
    # the executable contains content for CodeFlash #0 or #1, or WorkFlash #0 or 1
    content_in_code_flash0 = True if re.search('^\S+\s+\S+\s+(0x10|0x17)', map_cmd_output, re.MULTILINE) else False
    content_in_code_flash1 = True if re.search('^\S+\s+\S+\s+(0x18|0x1f)', map_cmd_output, re.MULTILINE) else False
    content_in_work_flash0 = True if re.search('^\S+\s+\S+\s+(0x14)',      map_cmd_output, re.MULTILINE) else False
    content_in_work_flash1 = True if re.search('^\S+\s+\S+\s+(0x1c)',      map_cmd_output, re.MULTILINE) else False

    ResetAndPrepareFlashOperation()

    if content_in_code_flash0:
        ExecFlashBurnCmd(elf_file, ConvertToHex(CODE_FLASH0_BASE_ADDRESS) + sflash_permission_string)
    if content_in_code_flash1:
        ExecFlashBurnCmd(elf_file, ConvertToHex(CODE_FLASH1_BASE_ADDRESS))
    if content_in_work_flash0:
        ExecFlashBurnCmd(elf_file, ConvertToHex(WORK_FLASH0_BASE_ADDRESS))
    if content_in_work_flash1:
        ExecFlashBurnCmd(elf_file, ConvertToHex(WORK_FLASH1_BASE_ADDRESS))

# Program flash with applications of all enabled cores
def ProgramFlash():
    # The 'map' command will print the path to the ELF file and section information for the core 
    # that is used to execute the command. The output will be parsed in ProgramApplication().

    if CM0_INCLUDED_IN_GHSMC_FILE is True:
        print("==================================")
        print("Programming CM0+ application image")
        ExecDebugCmdViaCm0("map")
        ProgramApplication(GetDebugCmdOutput())

    if CM7_0_INCLUDED_IN_GHSMC_FILE is True:
        print("===================================")
        print("Programming CM7_0 application image")
        ExecDebugCmdViaCm7_0("map")
        ProgramApplication(GetDebugCmdOutput())

    if CM7_1_INCLUDED_IN_GHSMC_FILE is True:
        print("===================================")
        print("Programming CM7_1 application image")
        ExecDebugCmdViaCm7_1("map")
        ProgramApplication(GetDebugCmdOutput())

    if CM7_2_INCLUDED_IN_GHSMC_FILE is True:
        print("===================================")
        print("Programming CM7_2 application image")
        ExecDebugCmdViaCm7_2("map")
        ProgramApplication(GetDebugCmdOutput())

    if CM7_3_INCLUDED_IN_GHSMC_FILE is True:
        print("===================================")
        print("Programming CM7_3 application image")
        ExecDebugCmdViaCm7_3("map")
        ProgramApplication(GetDebugCmdOutput())
...
if EXECUTION_MODE == "RAM":
    if IsCm0ResetVectorCatchWorking() is False:
        ...
        # may have decided to enter CM0+ user code depending on content at CODE_FLASH0_BASE_ADDRESS
        ...
        code_flash_base_content = ReadViaCm0(CODE_FLASH0_BASE_ADDRESS)
