...
static const stc_display_presets_t m_astcDispPresets[] =
{
    ...
    { .u16HorizontalActive =  640, .u16HorizontalTotal =  800, .u16VerticalActive = 480, .u16VerticalTotal = 500, .u8RefreshRateHz = 60, .bUseFpd = true, .u8MapIndex = MAP_INDEX_VESA },
    { .u16HorizontalActive =  640, .u16HorizontalTotal =  800, .u16VerticalActive = 480, .u16VerticalTotal = 500, .u8RefreshRateHz = 60, .bUseFpd = true, .u8MapIndex = MAP_INDEX_JEIDA },
    { .u16HorizontalActive = 1280, .u16HorizontalTotal = 1440, .u16VerticalActive = 720, .u16VerticalTotal = 750, .u8RefreshRateHz = 60, .bUseFpd = true, .u8MapIndex = MAP_INDEX_VESA },
    { .u16HorizontalActive = 1024, .u16HorizontalTotal = 1344, .u16VerticalActive = 600, .u16VerticalTotal = 625, .u8RefreshRateHz = 30, .bUseFpd = true, .u8MapIndex = MAP_INDEX_VESA },
    { .u16HorizontalActive =  720, .u16HorizontalTotal =  858, .u16VerticalActive = 480, .u16VerticalTotal = 525, .u8RefreshRateHz = 60, .bUseFpd = true, .u8MapIndex = MAP_INDEX_VESA },
    { .u16HorizontalActive =  720, .u16HorizontalTotal =  864, .u16VerticalActive = 576, .u16VerticalTotal = 625, .u8RefreshRateHz = 50, .bUseFpd = true, .u8MapIndex = MAP_INDEX_VESA },
};
...
static cy_gfxenv_en_result_t InitSmifCoreAndDeviceForS28H(stc_smif_cfg_internal_t * pstcSmifIntCfg, const cy_gfxenv_stc_smif_cfg_t * pstcSmifUsrCfg)
{
    ...
    // Some delay to ensure that device is out of reset (~600µs max): IMO max freq is 8.368 MHz --> ~5021 IMO clock cycles
    Cy_SysClk_StartClkMeasurementCounters(CY_SYSCLK_MEAS_CLK_IMO, 5020, CY_SYSCLK_MEAS_CLK_IMO);
    while(Cy_SysClk_ClkMeasurementCountersDone() == 0)
    {        
    }
...
static cy_gfxenv_en_result_t ResetS28HViaCsSignalingProtocol(uint8_t u8CoreNumber, uint8_t u8DeviceNumber)
{
    ...
    for(uint8_t u8Count = 0; u8Count < 4; u8Count++)
    {
        ...
        // should achieve >500ns confirm by measurement or change code to use proper time source
        for(uint8_t u8Delay = 0; u8Delay < 100; u8Delay++)
        {
            __ISB();
        }
